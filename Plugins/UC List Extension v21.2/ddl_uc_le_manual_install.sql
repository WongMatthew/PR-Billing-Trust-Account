create or replace package UC_LIST_EXTENSION as  
    ...
    * 27.05.2021  A. Grlica     Support for Item/Grid Lov Type (SQL Query and PL/SQL Function Body returning SQL Query)
    * 
    * </pre>
    * 
    * @headcom
...
c_version    constant varchar2(15 char) := '21.2.0.1';
...
begin
    ...
    then
        ...
        if l_item.display_as_code like 'NATIVE_POPUP_LOV' and l_item.lov_named_lov is null and l_item.lov_definition LIKE 'STATIC%'
        then
                apex_debug.error('%s Item "%s" error, Popup LOV item must be be dynamic type.', c_log_prefix, upper(p_dynamic_action.attribute_01));
                raise_application_error(-20004, c_log_prefix||' Item "'||upper(p_dynamic_action.attribute_01)||'" error, Popup LOV item must be be dynamic type.');    
        ...
        if l_column.lov_type_code like 'STATIC' and l_column.item_type like 'NATIVE_POPUP_LOV'
        then
                apex_debug.error('%s Column "%s" error, Popup LOV item must be be dynamic type.', c_log_prefix, upper(p_dynamic_action.attribute_07));
                raise_application_error(-20006, c_log_prefix||' Column "'||upper(p_dynamic_action.attribute_07)||'" error, Popup LOV item must be be dynamic type.');    
    ...
    then
        apex_debug.error('%s error LOV "%s" must be dynamic type.', c_log_prefix, l_item.lov_named_lov);
        raise_application_error(-20007, c_log_prefix||' error LOV "'||l_item.lov_named_lov||'" must be dynamic type.');  
    end if;
...
is
    ...
    -- This is used only when LOV Source is not inside shared components
    procedure parse_sql_source(p_query                in  apex_application_page_items.lov_definition%type,
                               p_location_code        out apex_application_lovs.location_code%type,
                               p_list_of_values_query out apex_application_lovs.list_of_values_query%type,
                               p_source_type_code     out apex_application_lovs.source_type_code%type,
                               p_return_column_name   out apex_application_lovs.return_column_name%type,
                               p_display_column_name  out apex_application_lovs.display_column_name%type)
    is
    ...
    begin
        p_location_code := 'LOCAL';
        p_list_of_values_query := p_query;
    
        if  lower(p_query) LIKE 'return%'  
        then
            p_source_type_code := 'FUNC_BODY_RETURNING_SQL';
            l_query := apex_plugin_util.get_plsql_function_result ( p_plsql_function => p_query );
        else
            p_source_type_code := 'SQL';
            l_query := p_query;
        end if;
        
        cur := dbms_sql.open_cursor;
        dbms_sql.parse(cur, l_query, dbms_sql.native);
        dbms_sql.describe_columns2(cur, columncount, describecolumns);      
        for i in 1..columncount loop
            if i = 1
            then
                p_display_column_name := describecolumns(i).col_name;
            elsif i = 2 
            then
                p_return_column_name := describecolumns(i).col_name;
            end if;
        end loop;
...
begin
    ...
    then
        ...
        if l_item.lov_named_lov is not null
        then
            set_lov;
        else
            parse_sql_source(p_query                => l_item.lov_definition,
                             p_location_code        => l_lov.location_code,
                             p_list_of_values_query => l_lov.list_of_values_query,
                             p_source_type_code     => l_lov.source_type_code,
                             p_return_column_name   => l_lov.return_column_name,
                             p_display_column_name  => l_lov.display_column_name);
        end if;
        ...
        if l_column.lov_type_code like 'SHARED'
        then
            set_lov;
        else
            parse_sql_source(p_query                => l_column.lov_source,
                             p_location_code        => l_lov.location_code,
                             p_list_of_values_query => l_lov.list_of_values_query,
                             p_source_type_code     => l_lov.source_type_code,
                             p_return_column_name   => l_lov.return_column_name,
